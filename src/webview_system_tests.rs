...
/// synchronous native stall would hang past it, so the `webview` nextest
/// group terminates this test's process at 60 s and fails it — a teardown
/// abort is observed as a failure, never masked as a pass.
#[test]

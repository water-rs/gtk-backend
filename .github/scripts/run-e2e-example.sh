...
# --release`, launches the staged binary directly, waits on the instrumented
# backend's per-surface render-completion events, and captures the window
# once readiness holds.
#
# THROWAWAY DIAGNOSTIC for water-rs/gtk-backend#44 — never merged to dev.
...
# optional RECORD=1 with RECORD_DIR to bank the capture as an artifact.
set -uo pipefail
...
# Content-readiness events keyed per example. The pattern must be an event
...
SETTLE_DEADLINE=90            # readiness deadline; exceeded → explicit FAIL
...
run_example() {
    ...
    # an explicit opt-in here. `waterui_map_gpu=debug` is required: the map
    # pipeline's readiness events — including "activated prepared GPU map" —
    # are all debug-level and were invisible in the blank-capture run.
    ...
    # Readiness gate. Every id is unique per widget instance, assigned at
    # construction by the instrumented backend; completion events fire only
    # after the render path finished for that id.
    created_surface_ids() {
    ...
    if ((record)); then
        echo "RECORD ${name} (readiness met)"
        return 0
    fi
    echo "PASS ${name} (readiness met)"
    return 0
}
